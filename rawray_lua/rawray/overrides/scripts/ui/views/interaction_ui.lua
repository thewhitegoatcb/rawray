if not rr.config or not rr.config.debugger or not rr.config.debugger.apply_new_overrides then
	return dofile("@scripts/ui/views/interaction_ui")
end
...
for _, dlc in pairs(DLCSettings) do
	...
	if interaction_ui_components then
		for _, component in pairs(interaction_ui_components) do
			fassert(not extended_components[component.name], "[InternactionUi] There is already a component with the name %q", component.name)
			local_require(component.filename)

			extended_components[component.name] = component.class_name
		end
	end
end
